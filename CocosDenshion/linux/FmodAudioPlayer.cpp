...
namespace CocosDenshion
{
	...
	FmodAudioPlayer::FmodAudioPlayer() : pMusic(0), pBGMChannel(0), iSoundChannelCount(0)
	{
		init();
	}
	...
	void FmodAudioPlayer::preloadBackgroundMusic(const char *pszFilePath)
	{
		...
		printf("FmodAudioPlayer::preloadBackgroundMusic() => 150");
		ERRCHECK(result);
	}
	...
	void FmodAudioPlayer::playBackgroundMusic(const char *pszFilePath, bool bLoop)
	{
		...
		FMOD_RESULT result = pSystem->playSound(pMusic, NULL, true,
												&pBGMChannel);
	...
	unsigned int FmodAudioPlayer::playEffect(const char *pszFilePath, bool bLoop)
	{
		...
		do
		{
			...
			if (l_it->first.empty() || l_it->first.compare("") == 0)
			{
			...
			FMOD_RESULT result = pSystem->playSound(pSound, pChannelGroup, true,
													&pChannel);
