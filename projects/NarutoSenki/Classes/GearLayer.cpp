...
void GearButton::click()
{
	...
	auto frame = getSrpiteFrame(CCString::createWithFormat("gearDetail_%02d.png", _gearType));
	...
	auto icon = getSrpiteFrame(CCString::createWithFormat("gear_%02d.png", _gearType));
...
void GearLayer::updateGearList()
{
	...
	for (int i = 0; i < _screwLayer->gearNum; i++)
	{
		...
		if (currentGear == None && !isBuyed)
		{
			...
			auto frame = getSrpiteFrame(CCString::createWithFormat("gearDetail_%02d.png", i));
