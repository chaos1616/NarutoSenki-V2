...
bool StartMenu::init()
{
	...
	do
	{
		...
		CCLabelBMFont *versionLabel = CCLabelBMFont::create("2.0 New Version", "Fonts/1.fnt");
...
void StartMenu::onGroupBtn(CCObject *sender)
{
	...
	if (!group_layer && !profile_layer)
	{
		...
		if (GroupID)
		{
			...
			request->setResponseCallback(this, httpresponse_selector(StartMenu::onGroupManageRequestCompleted));
		...
		else
		{
			...
			request->setUrl(url.c_str());
			request->setRequestType(CCHttpRequest::kHttpGet);
			request->setResponseCallback(this, httpresponse_selector(StartMenu::onGroupRequestCompleted));
			request->setTag("Post_My_Data");
			cocos2d::extension::CCHttpClient::getInstance()->send(request);
			request->release();
		}
...
void StartMenu::onQuitComfirm(CCObject *sender)
{
	...
	request->setUrl(url.c_str());
	request->setRequestType(CCHttpRequest::kHttpGet);
	request->setResponseCallback(this, httpresponse_selector(StartMenu::onQuitGroupRequestCompleted));
	request->setTag("Post_My_Data");
	cocos2d::extension::CCHttpClient::getInstance()->send(request);
	request->release();
}

void StartMenu::onQuitGroupRequestCompleted(CCHttpClient* client, CCHttpResponse* response)
{
	isPosting = false;
	if (refreshBtn)
	{
...
void StartMenu::onNoticeComfirm(CCObject *sender)
{
	...
	request->setUrl(url.c_str());
	request->setRequestType(CCHttpRequest::kHttpGet);
	request->setResponseCallback(this, httpresponse_selector(StartMenu::onNoticeRequestCompleted));
	request->setTag("Post_My_Data");
	cocos2d::extension::CCHttpClient::getInstance()->send(request);
	request->release();
}
...
void StartMenu::onLockBtn(CCObject *sender)
{
	...
	if (MemberID == leaderID)
	{
		if (isLock)
		{
			...
			request->setUrl(url.c_str());
			request->setRequestType(CCHttpRequest::kHttpGet);
			request->setResponseCallback(this, httpresponse_selector(StartMenu::onGPWRequestCompleted));
			request->setTag("Post_My_Data");
			cocos2d::extension::CCHttpClient::getInstance()->send(request);
			request->release();
		}
...
void StartMenu::onGPWRequestCompleted(CCHttpClient* client, CCHttpResponse* response)
{
...
void StartMenu::onGroupRequestCompleted(CCHttpClient* client, CCHttpResponse* response)
{
	...
	CCARRAY_FOREACH(_screwLayer1->getItemArray(), pObject)
	{
		...
		if (screwItem->getTag() == item->getTag())
		{
			...
			else
			{
				...
				request->setUrl(url.c_str());
				request->setRequestType(CCHttpRequest::kHttpGet);
				request->setResponseCallback(this, httpresponse_selector(StartMenu::onJoinRequestCompleted));
				request->setTag("Post_My_Data");
				cocos2d::extension::CCHttpClient::getInstance()->send(request);
				request->release();
				if (!refreshBtn)
...
void StartMenu::onJoinRequestCompleted(CCHttpClient* client, CCHttpResponse* response)
{
	isPosting = false;
	if (refreshBtn)
	{
...
void StartMenu::onSearchBtn(CCObject *sender)
{
	if (!isPosting)
	{
		...
		request->setUrl(url.c_str());
		request->setRequestType(CCHttpRequest::kHttpGet);
		request->setResponseCallback(this, httpresponse_selector(StartMenu::onSeachRequestCompleted));
		request->setTag("Post_My_Data");
		cocos2d::extension::CCHttpClient::getInstance()->send(request);
		request->release();
...
void StartMenu::onSeachRequestCompleted(CCHttpClient* client, CCHttpResponse* response)
{
...
void StartMenu::onCreateComfirm(CCObject *sender)
{
	...
	if (tempCoin >= 0)
	{
		...
		request->setUrl(url.c_str());
		request->setRequestType(CCHttpRequest::kHttpGet);
		request->setResponseCallback(this, httpresponse_selector(StartMenu::onCreateRequestCompleted));
		request->setTag("Post_My_Data");
		cocos2d::extension::CCHttpClient::getInstance()->send(request);
		request->release();
...
void StartMenu::onCreateRequestCompleted(CCHttpClient* client, CCHttpResponse* response)
{
	isPosting = false;
	if (refreshBtn)
	{
...
void StartMenu::onProfileBtn(CCObject *sender)
{
	...
	if (!group_layer && !profile_layer)
	{
		...
		request->setUrl(url.c_str());
		request->setRequestType(CCHttpRequest::kHttpGet);
		request->setResponseCallback(this, httpresponse_selector(StartMenu::onProfileRequestCompleted));
		request->setTag("Post_My_Data");
		cocos2d::extension::CCHttpClient::getInstance()->send(request);
		request->release();
		if (!refreshBtn)
...
void StartMenu::onNamePlateComfrim(CCObject *sender)
{
	...
	if (tempCoin >= 0)
	{
		...
		request->setUrl(url.c_str());
		request->setRequestType(CCHttpRequest::kHttpGet);
		request->setResponseCallback(this, httpresponse_selector(StartMenu::onNamePlateRequestCompleted));
		request->setTag("Post_My_Data");
		cocos2d::extension::CCHttpClient::getInstance()->send(request);
		request->release();
...
void StartMenu::onNamePlateRequestCompleted(CCHttpClient* client, CCHttpResponse* response)
{
	isPosting = false;
	if (refreshBtn)
	{
...
void StartMenu::onLoginOn(CCObject *sender)
{
	...
	// request->setUrl(url.c_str());
	// request->setRequestType(CCHttpRequest::kHttpGet);
	// request->setResponseCallback(this, httpresponse_selector(StartMenu::onLoginRequestCompleted));
	// request->setTag("Post_My_Data");
	// cocos2d::extension::CCHttpClient::getInstance()->send(request);
	// request->release();
	if (!refreshBtn)
	...
	// auto multiplayer = Multiplayer::create();
	// multiplayer->run([&]() {
	...
	// 	if (this->login_btn)
	// 	{
	// 		this->login_btn->removeFromParent();
	// 		this->login_btn = NULL;
	// 	}
