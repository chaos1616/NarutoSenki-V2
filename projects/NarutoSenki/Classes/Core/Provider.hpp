...
public:
	static Hero *create(CCString *character, CCString *role, CCString *group)
	{
		...
		is("Naruto") 						is_tag(ROLE_CLONE) ptr = new NarutoClone();		else ptr = new Naruto();
		is("SageNaruto") 					is_tag(ROLE_CLONE) ptr = new SageNarutoClone();	else ptr = new Naruto();
		is("RikudoNaruto") 					is_tag(ROLE_CLONE) ptr = new RikudoNarutoClone();	else ptr = new Naruto();
