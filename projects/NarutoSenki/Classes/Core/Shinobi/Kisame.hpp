	...
	void perform()
	{
		...
		if (_mainTarget && strcmp(_mainTarget->getRole()->getCString(), "Flog") != 0)
		{
			...
			if ((_actionState == State::IDLE || _actionState == State::WALK || _actionState == State::NATTACK) && (abs(sp.x) < 128 || _isCanGear00))
			{
				...
				if (_isCanOugis2 && !_isControlled && _delegate->_isOugis2Game && _mainTarget->getGP() < 5000 && !_mainTarget->_isBati && _mainTarget->getActionState() != State::KNOCKDOWN && !_mainTarget->_isSticking && !_skillChangeBuffValue)
				{
